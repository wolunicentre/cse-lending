<?php
	...
	function showCheckedOutItems() {
		$row = 0;
		global $confeed;
		$sql = 	"select t.tran_id, t.asset_id, t.person_id, t.cout, a.barcode, a.description" . 
				" from cse_trans t inner join cse_assets a on t.asset_id = a.asset_id  where cin is null";
		$sqlp = ociparse($confeed, $sql);
		if (ociexecute($sqlp, OCI_DEFAULT)) {
			echo "<div style='padding: 2px; border-bottom: 1px solid black; overflow: auto;'><div style='float: left; width: 30%'><h5>Name</h5></div><div style='width: 30%;float:left;'><h5>Item</h5></div></div>";
			while (ocifetchinto($sqlp, $data, OCI_BOTH)) {
				...
				if ($row % 2 == 1) {
					echo "<div style='background-color: #eee; padding: 2px; overflow: auto;'><div id=r".$row." style='float:left;display:none;'>".$tran."</div><div style='width: 30%;float:left;'>" 
						...
						. $data["DESCRIPTION"] . "</div><div style='float:left;width: 15%;'><a href='#' onClick='checkIn(r".$row.".innerHTML);'>Check In</a></div><div id='c".$row."' style='float:left;width: 15%;'></div></div>";
				} else {
	...
	function chksByItem() {
		...
		$sql = "select a.description as item, count(*) as count from cse_trans t " . 
				"inner join cse_assets a on t.asset_id = a.asset_id group by a.description";
		$sqlp = ociparse($confeed, $sql);
		if (ociexecute($sqlp, OCI_DEFAULT)) {
			while (ocifetchinto($sqlp, $data, OCI_ASSOC)) {
				if (!$r) {
